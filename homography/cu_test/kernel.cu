...
//for random sample, output: mask in the length of s_d_check with different index
...
//Input: dev_frames0x,...
//l_list: the length of dev_frames0x
//output: result_ninliers, result_MSS
...
int main(void) {
	...
	int2 *dev_results, *host_results, results;
	...
	int temp_count = 0, N_I_star = 0, *dev_result_ninliers, *host_result_ninliers, result_ninliers;
...
//	fclose(fp);
	...
	fprintf(fp, "%d\t %d\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t", card_core, ninliers, elapsedTime1, elapsedTime2, elapsedTime3, elapsedTime4, elapsedTime5, elapsedTime1 + elapsedTime2 + elapsedTime3 + elapsedTime4 + elapsedTime5, duration, proj_error);
	fclose(fp);
